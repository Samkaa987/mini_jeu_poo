...
class HumanPlayer < Player
	...
	def compute_damage
  	...
  	def search_weapon
  		result = rand(1..6)
  		puts "Tu cherches une nouvelle arme :[ENTER]"
  		print ">"
  		gets.chomp
  		puts "Tu viens de trouver une arme de niveau #{result} !"
  		if result > @weapon_level
  			@weapon_level = result
  			puts "Cette arme est meilleure que la tienne, tu la prends !"
  		else puts "Cette arme est nulle, tu ne la prends pas..."
  		end	
  	end

  	def search_health_pack
  		puts "Tu cherches un pack de points de vie :[ENTER]"
  		print ">"
  		gets.chomp
  		result = rand(1..6)
  		if result == 1
